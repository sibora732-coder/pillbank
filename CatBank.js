...
// ======== HELPERS ========
const toUnits = (amount) => BigInt(Math.floor(amount * 10 ** DECIMALS));
const fromUnits = (raw) => Number(raw) / 10 ** DECIMALS;
...
async function onMint() {
  ...
  if (!DEPLOYER_ADDRESS || DEPLOYER_ADDRESS.length < 10) {
    alert("DEPLOYER_ADDRESS not set. Update config in CatBank.js.");
    return;
  }
  try {
    const token = await getContract(TOKEN_ADDRESS);
